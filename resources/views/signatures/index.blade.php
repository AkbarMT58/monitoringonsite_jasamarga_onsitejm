    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                <div>
                    <h4 class="mb-3">Signatures List</h4>
                    <p class="mb-0">Aplikasi Signatures karyawan. </p>
                </div>
                <div>
                <a href="{{ route('create_signatures') }}" class="btn btn-primary add-list"><i class="fa-solid fa-plus mr-3"></i>Add Signatures</a>
            ...
            <div class="table-responsive rounded mb-3">
                <table class="table mb-0">
                    ...
                    <tbody class="ligth-body">
                        @forelse ($signatures as $signs)
                        <tr>
                            <td>{{$loop->iteration  }}</td>
                           
                            <td>{{ $signs->employee->name }}</td>
                            <td>{{ $signs->kategori_jabatan }}</td>
                            <td><img src="{{url('assets/images/dokumen/signatures/'.$signs->ttd_link)}}" width="100px"/></td>
                          
                            <td>
                                <div class="d-flex align-items-center list-action">
                                  
                                    <a class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"
                                        href="{{ route('signatures.edit', $signs->id) }}""><i class="ri-pencil-line mr-0"></i>
                                    </a>
                                    <form action="{{ route('signatures.destroy', $signs->id) }}" method="POST" style="margin-bottom: 5px">
                                        @method('delete')
                                        @csrf
